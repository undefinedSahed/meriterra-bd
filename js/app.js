...
function renderGallery() {
  ...
  itemsToRender.forEach((item, index) => {
    const el = document.createElement('div');
    el.className = 'project-item animate-on-scroll animate-delay-' + Math.min(index + 1, 4);
...
function initContactForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      formStatus.textContent = '✗ Failed to send message. Please email us directly at ops@mariterrabd.com';
